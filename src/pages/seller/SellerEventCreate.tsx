...
const CATEGORIES = ['컨퍼런스', '밋업', '해커톤', '스터디', '세미나', '워크샵', '기타']
...
const EMPTY_FORM: EventForm = {
  ...
  maxQuantityPerUser: '1', eventDateTime: '', location: '',
}
...
function EventForm({ initialData, onSubmit, submitLabel }: {
  ...
  const validate = () => {
    ...
    if (!form.eventDateTime) e.eventDateTime = '일시를 선택하세요'
    if (!form.location) e.location = '장소를 입력하세요'
    if (!form.totalQuantity || parseInt(form.totalQuantity) < 1) e.totalQuantity = '수량을 입력하세요'
    setErrors(e)
...
export default function SellerEventCreate() {
  ...
  const handleSubmit = async (form: EventForm) => {
    const res = await createSellerEvent({
      ...
      maxQuantityPerUser: parseInt(form.maxQuantityPerUser) || 1,
  ...
  useEffect(() => {
    ...
    getSellerEventDetail(id).then(res => {
      ...
      setInitial({
        ...
        techStacks: d.techStacks, price: String(d.price),
        totalQuantity: String(d.totalQuantity), maxQuantityPerUser: String(d.maxQuantityPerUser),
        eventDateTime: d.eventDateTime.slice(0, 16), location: d.location,
      })
