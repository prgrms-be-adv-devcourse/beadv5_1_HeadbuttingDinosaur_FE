...
| 이름 | 역할 | 위치 (파일) | 의존 컴포넌트 |
|---|---|---|---|
| `EventDetail` | 페이지 컨테이너. `useEventDetail(eventId)` 호출, loading/error/not-found/success 분기. 성공 시 좌측 콘텐츠 + 우측 `PurchasePanel` 의 2-column grid 조립. 본문 아래에 `RecommendedSection` 을 grid 밖 full-width 로 추가 (§ 8 항목 4). | `src/pages-v2/EventDetail/EventDetail.tsx` | `EventDetailSkeleton`, `Breadcrumb`, `HeroBanner`, `EventHeader`, `InfoCard`, `EventDescription`, `PurchasePanel`, `RecommendedSection` |
...
| `RecommendedSection` | 본문 아래 추천 섹션. `useRecommendedEvents()` 호출 → `events.length > 0` 일 때만 헤더 + 카드 grid 렌더. 로딩/실패/빈 결과는 섹션 자체를 안 그림 (본문 표시 안 막음). 헤더는 mono "// recommended" + "당신에게 맞는 이벤트". § 8 항목 4. | `src/pages-v2/EventDetail/components/RecommendedSection.tsx` | `RecommendedCard` |
| `RecommendedCard` | mini 카드 (180px+ 그리드 cell). `category` mono pill + 제목 (Link to detail) + 일시 + 가격. Cart v1 의 `RecommendedEventCard` 패턴을 v2 톤으로 재작성. quick-add 버튼 **없음** (§ 8 항목 4-(v) 결정). | `src/pages-v2/EventDetail/components/RecommendedCard.tsx` | (RouterLink) |
...
| `SectionHead` | `EventDescription` 의 "이벤트 소개" 헤더에 적용 가능 (§ 8-O9 추천: 단순 h2). `RecommendedSection` 헤더에는 적용 — mono "// recommended" + "당신에게 맞는 이벤트". |
...
| `RecommendedCard` | **페이지 전용 유지** | EventList 의 `EventCard` 와 정보 밀도가 다름 (mini 카드, 카테고리 + 제목 + 일시 + 가격 4필드만). 공유하면 `EventCard` 가 너무 다양한 size/variant 떠맡게 됨. Cart v2 작업 시 동일 패턴이 필요하면 그때 같이 추출. |
| `RecommendedSection` | **페이지 전용 유지** | EventDetail 외 사용처 후보 없음 (Cart v1 은 자체 섹션 보유). |
...
EventDetail 본문 렌더링은 두 호출 — `getEventDetail(eventId)` (본문 데이터) 와
`recommendEvents()` (본문 아래 추천 섹션, § 8 항목 4) — 이 필요하다. 두 호출은
서로 의존 관계 없이 **병렬**로 발사하고 각자 독립적으로 상태 관리한다. 구매
액션 (장바구니 추가 / 바로 구매) 의 `addCartItem` 은 본 페이지의 데이터 의존이
아니라 인터랙션 트리거이므로 § 6 에서 따로 다룬다.
...
recommendEvents()
  => Promise<AxiosResponse<unknown>>     // 시그니처에 제네릭 없음
// → GET /api/events/user/recommendations
// 응답 형태(Cart.tsx 사용 기준): { events: Array<{ eventId, title, price,
//                                  eventDateTime, category, ... }> }
```

INVENTORY § 2 의 페이지→호출 함수 매핑 (`EventDetail.tsx → getEventDetail,
addCartItem`) 에 **추천 호출 추가**. v2 는 INVENTORY 갱신 대상 — 본 plan
머지 후 매핑 테이블에 `recommendEvents` 한 줄 추가 필요.
...
| `recommendEvents` | `GET /api/events/user/recommendations` | (없음) | § 8 항목 4. 인증 무관 항상 호출, 실패는 swallow + 섹션 숨김 (Cart.tsx 패턴). 본문 fetch 와 **병렬**로 발사 — 추천이 늦거나 실패해도 본문은 정상 렌더. |
...
### 표 2-B — 추천 응답 매핑 (`recommendEvents`)

소스: `recommendEvents()` (`src/api/events.api.ts:102`). `src/api/types.ts` 에
정식 타입 정의 없음 — Cart.tsx 가 `any` 로 매핑하므로 본 plan 에서 v2 어댑터
용 좁은 타입을 새로 정의한다.

| 응답 필드 (예상) | RecommendedCardVM 필드 | 변환 |
|---|---|---|
| `events[].eventId` | `eventId: string` | 그대로. |
| `events[].title` | `title: string` | 그대로. |
| `events[].category` | `category: string` | 결측 시 `'추천'` 폴백 (Cart.tsx 패턴). |
| `events[].price` | `price: number` | 결측 시 `0`. 표시는 `price > 0 ? "${price.toLocaleString()}원" : "무료"`. |
| `events[].eventDateTime` | `eventDateTime: string` (ISO) | 그대로 + `dateLabel` 파생 (공유 헬퍼 `toDateTimeLabels`). 결측 시 표시 "일정 확인". |

> **카드 5장 제한 + 현재 이벤트 제외**:
> ```ts
> const cards = (api.events ?? [])
>   .filter(e => e.eventId !== currentEventId)
>   .slice(0, 5)
>   .map(toRecommendedCardVM);
> ```
> 어댑터 함수 시그니처는 `toRecommendedCards(api, currentEventId): RecommendedCardVM[]`.
> 응답 타입은 `RecommendationResponse = { events?: RawRecommendedEvent[] }`,
> `RawRecommendedEvent = { eventId: string; title: string; price?: number;
> eventDateTime?: string; category?: string; }` 로 어댑터 모듈 안에 좁게
> 정의 (보존 영역 `src/api/types.ts` 는 건드리지 않음).
...
- **`src/pages-v2/EventDetail/adapters.ts`** (신규)
  ...
  - `toRecommendedCards(api: RecommendationResponse, currentEventId: string): RecommendedCardVM[]` — 추천 매핑 (표 2-B).
  - `RecommendationResponse` / `RawRecommendedEvent` / `RecommendedCardVM` 타입 정의.
...
hook 은 그 후의 최종 상태만 본다. 본 표는 **본문 호출 (`getEventDetail`) 기준**.
추천 호출 (`recommendEvents`) 의 모든 에러는 try/catch 로 swallow 해 빈 배열로
폴백 → `RecommendedSection` 자체가 안 그려짐 (§ 5 의 추천 섹션 항목 참조).
...
### 6) 추천 섹션 hook (`useRecommendedEvents`) — § 8 항목 4

별도 hook `useRecommendedEvents(currentEventId)` 를 같은 `hooks.ts` 에 추가.
`useEventDetail` 과 **독립적으로 fetch + 상태 관리** (§ 3 표 1 의 병렬 발사).
간략 정책:

- 캐시: 모듈 레벨 단일 슬롯 (`recommendCache: { data, fetchedAt } | null`) +
  STALE 60s. eventId 별로 캐시 분리하지 않음 — 같은 사용자에게는 같은 추천이
  나온다고 가정하고 페이지 간 재사용. `currentEventId` 는 캐시 적중 후 필터링
  단계에서만 사용.
- 상태 union: `{ status: 'loading' } | { status: 'ready'; cards: RecommendedCardVM[] } | { status: 'hidden' }` —
  실패/빈 결과/비로그인은 모두 `'hidden'` 으로 흡수 (UI 가 섹션 자체를 안
  그리도록).
- AbortController 로 effect cleanup.
- `refetch` 는 노출 안 함 — § 5 의 추천 섹션 항목 (자동 갱신 정책 § 6 결정 대상)
  에 종속.
...
본문 호출과 **독립적으로** 처리. 본문이 어떤 상태이든 추천 섹션은 자체
판단으로 그릴지 말지를 결정. 본문이 loading/error/not-found 분기일 때는
**추천 섹션도 같이 안 그림** (페이지 골격이 본문 카드/에러 카드로 대체되어
"본문 아래" 라는 위치가 없어지므로). 본문이 success 일 때만 추천 섹션 분기에
들어감.

| `useRecommendedEvents` 상태 | 트리거 | 표시 | 사용자 액션 |
|---|---|---|---|
| `'loading'` | 첫 호출 in-flight | **섹션 자체 미렌더** (본문만 보이고 아래 공간 비움). 짧게 노출되므로 별도 placeholder 안 그림. | 없음. |
| `'ready'` (`cards.length > 0`) | 200 응답 + 현재 이벤트 제외 후 ≥1장 남음 | 본문 grid 아래 full-width 영역에 `RecommendedSection` 렌더 — `SectionHead` ("// recommended" + "당신에게 맞는 이벤트") + `auto-fit, minmax(180px, 1fr)` grid 에 `RecommendedCard` 5장 이내. | 카드 클릭 → 해당 이벤트 detail 로 이동 (`/events-v2/{eventId}`). 같은 페이지 내 navigate 라 본 hook 의 캐시는 재사용 (사용자 단위 캐시). |
| `'hidden'` | 호출 실패 (401/네트워크/5xx) **또는** 응답 `events.length === 0` **또는** 현재 이벤트 제외 후 0장 | 섹션 자체 미렌더. 토스트도 띄우지 않음 (실패가 본문 표시를 막지 않도록 조용히 흡수). | 없음. |

> **격리 원칙**: 추천 호출의 모든 실패는 `useRecommendedEvents` hook 내부의
> try/catch 에서 잡혀서 `'hidden'` 상태로 변환된다. 컨테이너는 hook 의
> union 만 보고 분기하므로 본문 분기 체인에 영향 없음. § 3 표 3 의 본문 에러
> 매핑과 완전히 분리됨.
...
1. `loading` (placeholder 유무에 따라 두 형태) — 추천 섹션 미렌더
2. `not-found` — 추천 섹션 미렌더
3. `forbidden` — 추천 섹션 미렌더
4. `error` — 추천 섹션 미렌더
5. `success` → 내부에서 `isSoldOut` / `canBuy` / 비로그인 액션 분기
   (모두 `success` 의 sub-state). **추천 섹션은 본 분기에서만 별도 hook
...
| N5 | `src/pages-v2/EventDetail/components/RecommendedSection.tsx` | § 8 항목 4 / § 5-(6). hook 의 `'ready'` 분기에서만 렌더. `SectionHead` + auto-fit grid + `RecommendedCard[]`. |
| N6 | `src/pages-v2/EventDetail/components/RecommendedCard.tsx` | mini 카드. category pill / title link / 일시 / 가격. quick-add 없음. |

**수정 (3):**

| # | 경로 | 변경 |
|---|---|---|
| M1 | `src/pages-v2/EventDetail/hooks.ts` | 스텁 본체 제거 → § 4 정식 구현 + `useRecommendedEvents` 추가. `useState<EventDetailQuery>` + `useEffect` + 모듈 Map (`detailCache`, LRU 12, STALE 60s) + `AbortController`. 응답 200→success / 404→not-found / 403 (PROFILE_NOT_COMPLETED 제외)→forbidden / 그 외→error. `refetch` 는 cache 삭제 + tick 증가. § 8-O6 추천안 (placeholder 채택) 적용 시 EventList `cache` 를 import 해 초기 state 의 `previous` 슬롯에 부분 VM 합성. **`useRecommendedEvents(currentEventId)` (§ 4-(6)) 추가** — 단일 슬롯 캐시 (`recommendCache`, STALE 60s) + AbortController + try/catch swallow. `'loading' / 'ready' / 'hidden'` 3상태. `toRecommendedCards` 어댑터 (§ 3 표 2-B) 사용. |
| M2 | `src/pages-v2/EventDetail/EventDetail.tsx` | PR 1 의 success-only 분기 → 분기 체인 추가 (§ 5 우선순위): `loading` (스켈레톤 또는 placeholder) → `not-found` (NotFoundCard) → `forbidden` (ForbiddenCard) → `error` (ErrorState + refetch) → `success` (기존 본문 + PurchasePanel + **`useRecommendedEvents` 호출 후 `'ready'` 분기일 때 `RecommendedSection`** § 5-(6)). Breadcrumb 는 모든 분기에서 노출 (제목 슬롯은 placeholder). |
...
| M4 | `src/pages-v2/EventDetail/adapters.ts` | `toRecommendedCards(api, currentEventId)` + `RecommendationResponse` / `RawRecommendedEvent` / `RecommendedCardVM` 타입 추가 (§ 3 표 2-B). PR 1 의 `toEventDetailVM` 본체는 그대로. |
...
| RecommendedSection / RecommendedCard (N5~N6) | ~110 |
| hooks.ts 정식 구현 + `useRecommendedEvents` (M1) | +90 (스텁 -25 → 정식 detail +85 + recommend hook +30) |
| EventDetail.tsx 분기 체인 + 추천 섹션 wire (M2) | +60 |
| adapters.ts 추천 매핑 추가 (M4) | +30 |
| index.tsx (M3) | 0~5 |
| App.tsx (A1) | +3 |
| __mocks__ 삭제 (D1) | -40 |
| **합계 (순증)** | **~440** |

사용자 목표 200~400 LOC 를 약간 초과 (~440). 추천 섹션 (~140 LOC) 이 한
PR 에 더해진 결과. 분리하고 싶으면 PR 4 (추천 섹션 단독, ~140 LOC) 로 떼는
것도 옵션 — § 9.4 참조.

> § 8-O6 의 placeholder 채택 시 EventList `hooks.ts` 의 `cache` export 추가
> (~5 LOC) + 본 hook 의 placeholder 합성 로직 (~30 LOC) 이 추가됨 → 총 ~475.
> placeholder 미채택 시 단순 스켈레톤만 노출.
...
3. **M4** `adapters.ts` 에 `toRecommendedCards` + 응답 타입 정의 추가.
4. **N6** `RecommendedCard.tsx` 신규 — RouterLink + mini 카드 마크업.
5. **N5** `RecommendedSection.tsx` 신규 — N6 + Phase 0 `SectionHead` 사용.
6. **M1** `hooks.ts` 정식 구현으로 교체 — 스텁 제거 + § 4 형태의 `useEventDetail`
   + § 4-(6) 의 `useRecommendedEvents` (M4 import). § 8-O6 채택 시 EventList
   cache import 추가.
7. **M2** `EventDetail.tsx` 분기 체인 추가 — N1~N4 import + hook union 분기 +
   success 분기 안에 N5 추가.
8. **M3** `index.tsx` 정리 — PR 1 의 mock 의존 잔재 있으면 제거 (대체로 변경
   불필요).
9. **D1** `__mocks__/sampleEventDetail.ts` 삭제 — 사용처 없는 것 확인 후 디렉
   토리째 삭제. `npm run build` 가 통과하면 안전.
10. **A1** `src/App.tsx` 한 줄 교체 — PR 1 검증용 임시 wire 와 동일 형태로 정식
    등록.
11. **(검증 후 commit)** 시각 검증 케이스 통과 확인.
...
| C3 | 3, 4, 5 | `feat(event-detail-v2): add RecommendedSection / Card and adapter` |
| C4 | 6 | `feat(event-detail-v2): replace stub hook with live useEventDetail and useRecommendedEvents` (placeholder 채택 시 메시지에 `+ EventList cache placeholder` 추가) |
| C5 | 7 | `feat(event-detail-v2): wire container to live data with state branches and recommended section` |
| C6 | 8, 9 | `chore(event-detail-v2): drop mock fixture and tidy index.tsx` |
| C7 | 10 | `feat(event-detail-v2): register VersionedRoute for /events/:id` |

7 commits 가 부담스러우면 C1~C3 통합 (`feat(event-detail-v2): add skeleton,
error cards, and recommended section`), C4~C6 통합 (`feat(event-detail-v2):
integrate live API and state handling`), C7 단독 — 총 3 commits 도 가능.
...
| 13 | success 분기에서 추천 응답 정상 (≥1장, 현재 이벤트 외) | 본문 아래 `RecommendedSection` 노출 — 헤더 + 카드 5장 이내. 카드 클릭 시 해당 detail 로 이동 + 새 detail 의 추천 섹션도 동일 캐시에서 즉시 렌더 (60s 안). |
| 14 | 추천 응답이 빈 배열 또는 현재 이벤트 1장만 포함 | `RecommendedSection` 자체 미렌더 (본문 grid 만 보임). |
| 15 | 추천 호출 401 / 5xx / 네트워크 차단 시 | 토스트/에러 카드 없이 조용히 섹션 미렌더. **본문 success 표시는 정상** (격리 확인). |
| 16 | 비로그인 상태로 detail 진입 | 본문 success + 추천 섹션은 401 swallow 로 미렌더 (대부분 케이스). |

**검증 결과 기록**: 16케이스를 PR 본문에 체크리스트로 옮기고 OK 표시. 케이스
4 (404), 5 (네트워크), 6 (재시도), 13 (추천 정상), 15 (추천 격리) 각각 스크린
샷 1장 권장.
...
| **`recommendEvents` (§ 8-항목 4)** | 이미 `src/api/events.api.ts:102` 에 존재하고 Cart v1 (`src/pages/Cart.tsx:47`) 이 사용 중. v2 추가 작업은 호출 + 어댑터 + 컴포넌트만. | 별도 외부 PR 의존 없음. **PR 2 에 흡수** (§ 9.2 의 N5/N6/M4 + M1 의 hook 추가). 또는 § 9.4 의 PR 4 분리 옵션 참조. |
...
3. PR 2 (live API + router + 추천 섹션) 머지 + ?v=2 케이스 16 통과
4. PR 3 (구매 플로우) 머지 + 케이스 8 통과   ← 분리 시
```

#### PR 4 (추천 섹션 분리) 옵션

§ 9.2 의 추천 섹션 작업 (~140 LOC: N5/N6/M4 + M1 의 `useRecommendedEvents` +
M2 의 wire) 을 PR 2 에서 떼서 PR 4 로 분리하는 옵션:

```
1. (EventList) → 2. PR 1 → 3. PR 2 (추천 제외) → 4. PR 3 (구매) ║ 4'. PR 4 (추천 섹션)
                                                                  ↑ PR 2 와 병렬 가능
```

- 장점: PR 2 가 ~300 LOC 로 줄어 budget 안. 추천 호출 실패 회귀가 본문 호출
  분기와 격리되어 리뷰 분리.
- 단점: 4 PR 모두 머지가 cross-team 조율 필요. 추천 섹션이 본문보다 늦게
  들어오는 동안 detail 페이지가 추천 없이 렌더 (사용자 가시 차이).
- **결정 기본값: 통합 (PR 2 흡수)**. 추천 작업이 격리도가 높아 (별도 hook +
  별도 컴포넌트 + 어댑터에 새 함수 추가만) 한 PR 안에서 충분히 안전하게
  리뷰 가능. 분리하려면 § 9.4 결정 시점에 합의.
