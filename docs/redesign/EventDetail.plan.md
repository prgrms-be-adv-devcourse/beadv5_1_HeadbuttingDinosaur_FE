...
| 4 | 관련/추천 이벤트 섹션 | (a) 본 PR 시리즈 범위 밖 / (b) 이번에 같이 | **미정 — 추천: (a) 본 시리즈 범위 밖** | 사용자 이전 응답 "장바구니에 추가" 는 항목 1 의 두 버튼 동작에 대한 답변이었음이 확인됨 → 본 항목 (관련/추천 섹션 도입) 은 아직 답변 없음. 추천안은 (a) — 프로토타입 EventDetail 에 해당 섹션 없음 + 추천 API 설계가 본 시리즈 범위 밖. 별도 결정 들어오면 갱신. |
...
- ✅ 항목 1, 2, 3, 5, 6 — 사용자 결정으로 확정
- ⏳ **항목 4 (관련/추천 이벤트 섹션)** — 미정. 추천안 (a) 본 시리즈 범위 밖으로 진행 가정.
